...
let state = { total: 0, events: {}, kinds: {}, labels: {}, files: {}, routes: {}, days: {} };
...
const server = createServer((req, res) => {
  ...
  req.on("end", async () => {
    try {
      ...
      state.total += 1; increment(state.events, event);
      if (event === "label_selected") increment(state.labels, kind); else if (event === "file_selected") increment(state.files, kind); else increment(state.kinds, kind);
      increment(state.routes, route); increment(state.days, new Date().toISOString().slice(0, 10));
